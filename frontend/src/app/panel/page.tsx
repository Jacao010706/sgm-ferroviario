...
function GeneratorSVG({ mode, fuelLevel, gridVoltage, voltageL1, running, temp, battery, noComm }: { mode: any, fuelLevel: any, gridVoltage: any, voltageL1: any, running: boolean, temp: any, battery: any, noComm?: boolean }) {
  const color = noComm ? "#ffd700" : running ? "#9333ea" : "#4488ff";
  ...
  return (
    <svg viewBox="0 0 200 300" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
      ...
      <circle cx="20" cy="244" r="5" fill={noComm?"#ffd700":running?"#9333ea":"#333"} stroke={color} strokeWidth="0.8"/>
      <text x="30" y="248" fill={color} fontSize="8" fontFamily="monospace">{mode===1?"AUTO":mode===0?"MANUAL":"---"}</text>
      <text x="190" y="248" textAnchor="end" fill={noComm?"#ffd700":running?"#9333ea":"#666"} fontSize="8" fontFamily="monospace">{noComm?"SEM SINAL":running?"OPERANDO":"PARADO"}</text>
        ...
        {[row1,row2].map((row,rowIdx)=>(
          <div key={rowIdx} className="flex-1 grid gap-2" style={{gridTemplateColumns:"repeat(13,1fr)"}}>
            {row.map((station)=>{
              ...
              const borderColor=noComm?"#ffd700":running?"#9333ea":"#4488ff";
              ...
              return (
                <div key={station.code} className="flex flex-col rounded cursor-pointer transition-all hover:brightness-125"
                  ...
                  <div className="flex items-center justify-between px-2 py-1" style={{background:"#0f0f0f",borderBottom:`1px solid ${borderColor}33`}}>
                    ...
                    <div className="w-2 h-2 rounded-full" style={{background:noComm?"#ffd700":running?"#9333ea":"#4488ff"}}/>
                  </div>
