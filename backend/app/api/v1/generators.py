...
async def _auditar(db, asset_id, tag, tipo, usuario, action, resultado, mensagem_erro=None):
    ...
    Se a gravacao falhar, o erro vai para o log do backend.
    """
    try:
...
async def comando_gerador(
    ...
    await _auditar(db, asset_id, tag, tipo, current_user.email, body.action, "tentativa")
    ...
    if not coletor_url:
        try:
            ...
            pass
    if not coletor_url:
        await _auditar(db, asset_id, tag, tipo, current_user.email, body.action,
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if r.status_code == 200:
                await _auditar(db, asset_id, tag, tipo, current_user.email, body.action, "sucesso")
            ...
            await _auditar(db, asset_id, tag, tipo, current_user.email, body.action, "falha", detail)
