...
const emptyForm = { code: "", name: "", description: "", unit: "un", quantity_stock: 0, quantity_minimum: 0, unit_cost: "", supplier: "", category: "" };
...
export default function PartsPage() {
  ...
  const load = () => {
    setLoading(true);
    api.get("/parts/", { params: { limit: 500 } }).then((r) => {
      setParts(r.data);
      const cats = Array.from(new Set(r.data.map((p: any) => p.category).filter(Boolean))) as string[];
  ...
  const filtered = parts.filter(p => {
    const matchCat = activeCategory === "Todas" || p.category === activeCategory || (activeCategory === "Sem categoria" && !p.category);
  ...
  const allTabs = ["Todas", ...categories, ...(parts.some(p => !p.category) ? ["Sem categoria"] : [])];
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 p-6 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          <div><h1 className="text-2xl font-bold text-slate-800">Pecas e Materiais</h1><p className="text-slate-500 text-sm">{filtered.length} de {parts.length} registros</p></div>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {allTabs.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={clsx("px-4 py-2 rounded-lg text-sm font-medium border transition-colors", activeCategory === cat ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50")}>
              {cat} <span className="ml-1 text-xs opacity-70">({cat === "Todas" ? parts.length : parts.filter(p => cat === "Sem categoria" ? !p.category : p.category === cat).length})</span>
            </button>
          ))}
        </div>
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">Carregando...</td></tr>
              : filtered.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400"><div className="flex flex-col items-center gap-2"><Package size={32} className="text-slate-300" /><span>Nenhuma peca encontrada</span></div></td></tr>
              : filtered.map(p => (
                <tr key={p.id} className={clsx("hover:bg-slate-50", p.quantity_stock <= p.quantity_minimum && p.quantity_minimum > 0 && "bg-red-50")}>
                  ...
                  <td className="px-4 py-3"><span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full text-xs">{p.category || "-"}</span></td>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div><label className={lbl}>Codigo *</label><input className={inp} value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} placeholder="Ex: SUB-001" /></div>
                  <div><label className={lbl}>Categoria</label><input className={inp} value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} placeholder="Ex: GGD, Subestacao" /></div>
                </div>
                <div><label className={lbl}>Nome *</label><input className={inp} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Ex: Disjuntor 100A" /></div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className={lbl}>Custo Unit. (R$)</label><input className={inp} type="number" step="0.01" value={form.unit_cost} onChange={e => setForm({ ...form, unit_cost: e.target.value })} /></div>
