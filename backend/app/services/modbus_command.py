...
def _escrever_dse_key(ip, slave_id, action):
    ...
    try:
        ...
        result = client.write_registers(address=DSE_REG_CONTROL_KEY, values=[key, complement], slave=slave_id)
        if result.isError():
...
def _escrever_st2160_bit(client, slave_id, bit, descricao):
    valor = 1 << bit
    result = client.write_registers(address=ST2160_REG_COMANDO, values=[valor], slave=slave_id)
    if result.isError():
...
def _enviar_stemac(ip, slave_id, action):
    ...
    try:
        if not client.connect():
            raise ComandoError(f"Sem conexao Modbus com {ip}")
        result = client.write_registers(address=ST2160_REG_SENHA, values=[ST2160_SENHA_OPERADOR], slave=slave_id)
        if result.isError():
