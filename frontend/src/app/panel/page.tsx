...
import { panelApi as api } from "@/lib/panelApi";
...
function GeneratorSVG({ mode, fuelLevel, gridVoltage, voltageL1, running, temp, battery, noComm }: { mode: any, fuelLevel: any, gridVoltage: any, voltageL1: any, running: boolean, temp: any, battery: any, noComm?: boolean }) {
  const color = noComm ? "#ffd700" : running ? "#ff3333" : "#4488ff";
  ...
  return (
    <svg viewBox="0 0 200 300" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
      ...
      <circle cx="20" cy="244" r="5" fill={noComm?"#ffd700":running?"#ff3333":"#333"} stroke={color} strokeWidth="0.8"/>
      <text x="30" y="248" fill={color} fontSize="8" fontFamily="monospace">{mode===1?"AUTO":mode===0?"MANUAL":"---"}</text>
      <text x="190" y="248" textAnchor="end" fill={noComm?"#ffd700":running?"#ff3333":"#666"} fontSize="8" fontFamily="monospace">{noComm?"SEM SINAL":running?"OPERANDO":"PARADO"}</text>
...
export default function PanelPage() {
  ...
  const [msgSenha, setMsgSenha] = useState("Senha incorreta!");
  const [entrando, setEntrando] = useState(false);

  // A senha vai ao servidor, que a confere contra CCO_SENHA e devolve um
  // cookie httpOnly. Nada de comparar senha dentro do navegador.
  const entrar = async () => {
    setEntrando(true); setErroSenha(false);
    try {
      ...
      if (r.ok) { setAuth(true); return; }
      const d = await r.json().catch(() => ({}));
      setMsgSenha(r.status === 503
        ? "Servidor sem a senha do CCO configurada."
        : (d?.erro || "Senha incorreta!"));
      setErroSenha(true); setSenha("");
    } catch {
      setMsgSenha("Nao foi possivel falar com o servidor.");
      setErroSenha(true);
    } finally {
      setEntrando(false);
    }
    ...
    <div className="min-h-screen bg-black flex items-center justify-center font-mono" style={{background:"#050505"}}>
      <div className="border border-green-800 rounded p-8 w-80" style={{background:"#0a0a0a"}}>
        ...
        <div className="mb-4">
          ...
          {erroSenha && <p className="text-red-500 text-xs mt-1">{msgSenha}</p>}
        </div>
        ...
        {[row1,row2].map((row,rowIdx)=>(
          <div key={rowIdx} className="flex-1 grid gap-2" style={{gridTemplateColumns:"repeat(13,1fr)"}}>
            {row.map((station)=>{
              ...
              const timestamps=assetLatest?Object.values(assetLatest).map((r:any)=>new Date(r.timestamp+"Z").getTime()).filter((t)=>!isNaN(t)):[];
                ...
                <div key={station.code} className="flex flex-col rounded cursor-pointer transition-all hover:brightness-125"
                  ...
                  <div className="flex items-center justify-between px-2 py-1" style={{background:"#0f0f0f",borderBottom:`1px solid ${borderColor}33`}}>
                    <div className="flex items-center gap-1">
                      <span className="font-bold" style={{color:typeColor,fontSize:"20px"}}>{station.code}</span>
                      <span className="font-bold" style={{color:typeColor,fontSize:"8px",opacity:0.85}}>{typeLabel}</span>
                    </div>
                    <span className="text-xs" style={{color:borderColor,fontSize:"9px"}}>{mode===1?"AUTO":mode===0?"MAN":"---"}</span>
                    <div className="w-2 h-2 rounded-full" style={{background:noComm?"#ffd700":running?"#ff3333":"#4488ff"}}/>
                  </div>
                  <div className="flex items-center justify-center p-1" style={{height:"calc(100% - 48px)"}}>
                    <GeneratorSVG mode={mode} fuelLevel={fuel} gridVoltage={gridV} voltageL1={voltL1} running={running} temp={temp} battery={battery} noComm={noComm}/>
                  </div>
